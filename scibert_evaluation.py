...
CNN, LSTM, Simple RNN, CNN-LSTM and Bidirectional LSTM are trained on the scibert text
...
documentation_file_parameteropt = open("parameter_optimization_scibert.txt", "w+")
documentation_file_modelopt = open("classifier_optimization_scibert.txt", "w+")
...
for i, line in enumerate(dataframe):
    ...
    preprocessed_tuple = (final_dataset, query)
    preprocessed_tuple_single = (dataset, query)
...
q_scibert = preprocessing.sci_bert_embeddings(queries)
documentation_file_parameteropt.write("scibert Evaluation \n")
documentation_file_modelopt.write("scibert Evaluation \n")

#split data in training and test data
d_train, d_test, q_train, q_test = train_test_split(datasets, q_scibert, test_size=0.2)
...
pickle.dump(label_encoder, open('label_encoder_scibert.sav', 'wb'))
...
pickle.dump(svm_model, open("svm_scibert.sav", 'wb'))
...
pickle.dump(rf_model, open("rf_model_scibert.sav", 'wb'))
...
pickle.dump(lr_model, open("lr_scibert.sav", 'wb'))
...
pickle.dump(gnb_model, open("gnb_scibert.sav", 'wb'))
...
d_train_single, d_test_single, q_train_single, q_test_single = train_test_split(
    datasets_single, q_scibert, test_size=0.2)
...
pickle.dump(label_binarizer, open("label_binarizer_scibert.sav", 'wb'))
...
lstm_scan = talos.Scan(x=X, y=d_train_array, model=lstm_optimization, params=lstm_params,
                       experiment_name='LSTM_Optimization_scibert', round_limit=10,
                       fraction_limit=0.05)
...
#deploy best model
model_lstm = talos.Deploy(lstm_scan, "model_lstm_scibert", metric='val_accuracy')
...
simple_rnn_scan = talos.Scan(x=X, y=d_train_array, model=simple_rnn_optimization,
                             params=simple_rnn_params, experiment_name='RNN_Optimization_scibert',
...
#deploy best model
model_simple_rnn = talos.Deploy(simple_rnn_scan, "model_simple_rnn_scibert", metric='val_accuracy')
...
cnn_lstm_scan = talos.Scan(x=X, y=d_train_array, model=cnn_lstm_optimization,
                           params=cnn_lstm_params, experiment_name='CNN_LSTM_Optimization_scibert',
                           round_limit=10, fraction_limit=0.05)
...
#deploy best model
model_cnn_lstm = talos.Deploy(cnn_lstm_scan, "model_cnn_lstm_scibert", metric='val_accuracy')
...
bi_lstm_scan = talos.Scan(x=X, y=d_train_array, model=bidirectional_lstm_optimization,
                          ...
                          experiment_name='Bidirectional_LSTM_Optimization_scibert',
                          round_limit=10, fraction_limit=0.05)
...
#deploy best model
model_bi_lstm = talos.Deploy(bi_lstm_scan, "model_bi_lstm_scibert",
                             metric='val_accuracy')

...
documentation_file_parameteropt = open("parameter_optimization_tfidf.txt", "w+")
documentation_file_modelopt = open("classifier_optimization_tfidf.txt", "w+")
...
classifier_rf.fit(q_train, d_train_encoded)
documentation_file_parameteropt.write(
...
pickle.dump(rf_model, open("rf_model_tfidf.sav", 'wb'))
pred_rf = rf_model.predict(q_test)
#evaluate the model, for abstracts use multilabel_evaluation_multilabelbinarizer() for citations
#use multilabel_evaluation()
...
classifier_lr.fit(q_train, d_train_encoded)
documentation_file_parameteropt.write(
...
pickle.dump(lr_model, open("lr_tfidf.sav", 'wb'))
pred_lr = lr_model.predict(q_test)
#evaluate the model, for abstracts use multilabel_evaluation_multilabelbinarizer() for citations
#use multilabel_evaluation()
...
q_train_gnb = q_train.toarray()
q_test_gnb = q_test.toarray()
gnb_model = OneVsRestClassifier(GaussianNB()).fit(q_train_gnb, d_train_encoded)
pickle.dump(gnb_model, open("gnb_tfidf.sav", 'wb'))
...
mnb_dict = dict(estimator__alpha=[1, 2, 5, 10, 50])
classifier_mnb = RandomizedSearchCV(estimator=OneVsRestClassifier(MultinomialNB()),
                                    param_distributions=mnb_dict,
                                    n_iter=5, n_jobs=1)
classifier_mnb.fit(q_train, d_train_encoded)
documentation_file_parameteropt.write(
    "Multinomial Naive Bayes: Best parameters {}, reached score: {} \n".format(
        classifier_mnb.best_params_, classifier_mnb.best_score_))
mnb_model = classifier_mnb.best_estimator_
...
classifier_cnb = RandomizedSearchCV(estimator=OneVsRestClassifier(ComplementNB()),
...
#split data in training and test data
d_train_single, d_test_single, q_train_single, q_test_single = train_test_split(datasets_single,
                                                                                q_tfidf,
                                                                                test_size=0.2)
...
pickle.dump(label_binarizer, open('label_binarizer_tfidf.sav', 'wb'))
x_train_array = q_train.toarray()
X = x_train_array[:, :, None]
x_test_array = q_test.toarray()
x_t = x_test_array[:, :, None]
...
#deploy best model
model_cnn = talos.Deploy(cnn_scan, "model_cnn_tfidf", metric='val_accuracy')
...
lstm_scan = talos.Scan(x=X, y=d_train_array, model=lstm_optimization, params=lstm_params,
                       experiment_name='LSTM_Optimization_tfidf', round_limit=10,
                       fraction_limit=0.05)
...
#deploy best model
model_lstm = talos.Deploy(lstm_scan, "model_lstm_tfidf", metric='val_accuracy')
...
simple_rnn_scan = talos.Scan(x=X, y=d_train_array, model=simple_rnn_optimization,
                             params=simple_rnn_params, experiment_name='RNN_Optimization_tfidf',
...
#deploy best model
model_simple_rnn = talos.Deploy(simple_rnn_scan, "model_simple_rnn_tfidf", metric='val_accuracy')
...
cnn_lstm_scan = talos.Scan(x=X, y=d_train_array, model=cnn_lstm_optimization,
                           params=cnn_lstm_params, experiment_name='CNN_LSTM_Optimization_tfidf',
                           round_limit=10, fraction_limit=0.05)
...
#deploy best model
model_cnn_lstm = talos.Deploy(cnn_lstm_scan, "model_cnn_lstm_tfidf", metric='val_accuracy')
...
bi_lstm_scan = talos.Scan(x=X, y=d_train_array, model=bidirectional_lstm_optimization,
                          ...
                          experiment_name='Bidirectional_LSTM_Optimization_tfidf',
                          round_limit=10, fraction_limit=0.05)
...
#deploy best model
model_bi_lstm = talos.Deploy(bi_lstm_scan, "model_bi_lstm_tfidf",
                             metric='val_accuracy')
